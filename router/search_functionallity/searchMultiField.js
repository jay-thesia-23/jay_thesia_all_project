...
app.get("/search",authentication, function (req, res) {
  ...
  let finalsql = `select * from stu_express where ${sqlfname} AND LastName LIKE '%${searchLname}%' AND city LIKE '%${searchCity}%' AND MobileNo LIKE '%${searchMoblie}%' AND collegeName LIKE '%${searchCollege}%' AND email LIKE '%${searchEmail}%';`;
